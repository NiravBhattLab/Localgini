function weights = getINITweights_lt2(modelData,model,lowerThs,upperThs)
...
if lowerThs > upperThs || lowerThs < 0 || upperThs < 0
    ...
    weights= thrRxnData.value;
    weights(weights<5*log(2))=-8;
    weights(weights>=5*log(2))=weights(weights>=5*log(2))/(5*log(2));
end