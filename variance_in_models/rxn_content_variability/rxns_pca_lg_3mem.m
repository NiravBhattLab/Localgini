...
load('../../CSMs_construction/generic_models/786O.mat')
...
thr = 'Localgini'; %%%% only change here
mems = {'FASTCORE','MBA','mCADRE'};

pca_mat =zeros(44*3,numel(model.rxns));
count =1;
for m=1:3
    mem = mems{m};
    for i=1:44
        load(['../../CSMs_construction/CancerCellLines/csm_models/',thr,'/',...
                mem,'/',cell_lines{i},'.mat'])
        pca_mat(count,:)=ismember(model.rxns,m1.rxns);
        count =count+1;
        clear m1
    end
end
...
pca_mat = pca_mat(:,sum(pca_mat,1)~=44*3);
...
all_poss_order = perms(1:3);
for i =1:size(all_poss_order,1)
    curr_order = all_poss_order(i,:);
    var_1 = [ones(44,1)*curr_order(1);ones(44,1)*curr_order(2);ones(44,1)*curr_order(3)];
...
save('3mem/LG_var_rxn_content_3mem','explained_var_in_pc1_mem','explained_var_in_pc2_mem','explained_var_in_pc3_mem','explained_var_in_pc1_cl','explained_var_in_pc2_cl','explained_var_in_pc3_cl','explained_var_in_pc1_ct','explained_var_in_pc2_ct','explained_var_in_pc3_ct')